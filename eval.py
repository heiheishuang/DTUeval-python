...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_output', type=str, default='zys_outputs')
    ...
    for file_name in ply_list:
        ...
        scan = file_name[6:-7]
        data = os.path.join(args.data_output, file_name)
        if args.mode == 'mesh':
            pbar = tqdm(total=9)
        ...
        write_vis_pcd(f'{args.data_output}/scan{int(scan)}/vis_{int(scan):03}_d2s.ply', data_down, data_color)
        stl_color = np.tile(B, (stl.shape[0], 1))
        stl_alpha = dist_s2d.clip(max=vis_dist) / vis_dist
        stl_color[np.where(above)[0]] = R * stl_alpha + W * (1 - stl_alpha)
        stl_color[np.where(above)[0][dist_s2d[:, 0] >= max_dist]] = G
        write_vis_pcd(f'{args.data_output}/scan{int(scan)}/vis_{int(scan):03}_s2d.ply', stl, stl_color)
